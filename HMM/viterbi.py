...
def modified_viterbi_algorithm(
    features: List[int],
    transition_params: np.ndarray,
    emission_params: np.ndarray,
    k: int = 1,
    ...
    for s in states:
        out[0][s].append(
            (
                start[0][s] + emission_params_log[s, features[0]],
                tuple(),
            )
        )
        ...
        for s in states:
            ...
            for prev_state, prev_state_scores in enumerate(out[i - 1]):
                for j, entry in enumerate(prev_state_scores):
                    ...
                    score = (
                        prev_score
                        + transition_params_log[
                            prev_state_path[-2] if len(prev_state_path) > 2 else 0
                        ][prev_state_path[-1] if len(prev_state_path) > 2 else 0][s]
                        + emission_params_log[s, features[i]]
                    )
                    h.append((score, tuple(prev_state_path) + (prev_state,)))
